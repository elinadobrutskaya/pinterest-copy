...
const REMOVE_CARD_FROM_BOARD = 'REMOVE_CARD_FROM_BOARD'
    ...
    case REMOVE_CARD_FROM_BOARD: {
      ...
      return {
        ...
        boards: state.boards.map((b) => {
          ...
          return {
            ...b,
            cards: b.cards.filter((c) => String(c.id) !== String(cardId)),
          }
        }),
      }
    }
...
// initial state
const initialState = { boards: [], cardsData: [], hiddenCards: [] }
...
export function removeCardFromBoard(boardId, cardId) {
  store.dispatch({ type: REMOVE_CARD_FROM_BOARD, payload: { boardId, cardId } })
}
